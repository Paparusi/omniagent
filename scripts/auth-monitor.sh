...
#
# Suggested cron: */30 * * * * /home/admin/omniagent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OmniAgent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., omniagent-alerts)
...
STATE_FILE="$HOME/.omniagent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OmniAgent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use omniagent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OmniAgent to $NOTIFY_PHONE..."
            omniagent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OmniAgent is down. Run: ssh l36 '~/omniagent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
