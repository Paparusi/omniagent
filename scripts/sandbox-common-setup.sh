...
BASE_IMAGE="${BASE_IMAGE:-omniagent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-omniagent-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=omniagent.sandbox=1)
NOTE
