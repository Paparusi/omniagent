...
# Build and bundle OmniAgent into a minimal .app we can open.
# Outputs to dist/OmniAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OmniAgent.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OmniAgent"
BUNDLE_ID="${BUNDLE_ID:-ai.omniagent.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/omniagent/omniagent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OmniAgent/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OmniAgentBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OmniAgentGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OmniAgent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OmniAgent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OmniAgent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OmniAgent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OmniAgent/Resources/OmniAgent.icns" "$APP_ROOT/Contents/Resources/OmniAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OmniAgent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OmniAgentKit resources"
OMNIAGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OmniAgentKit_OmniAgentKit.bundle"
if [ -d "$OMNIAGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OmniAgentKit_OmniAgentKit.bundle"
  cp -R "$OMNIAGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OmniAgentKit_OmniAgentKit.bundle"
else
  echo "WARN: OmniAgentKit resource bundle not found at $OMNIAGENTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OmniAgent"
killall -q OmniAgent 2>/dev/null || true
