...
const shouldBuild = (deps) => {
  if (deps.env.OMNIAGENT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OMNIAGENT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[omniagent] ${message}\n`);
};

const runOmniAgent = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["omniagent.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runOmniAgent(deps);
  }
