#!/bin/bash
# Setup OmniAgent Auth Management System
...
echo "=== OmniAgent Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'omniagent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OmniAgent message: Send warning via OmniAgent itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/omniagent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/omniagent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/omniagent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now omniagent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status omniagent-auth-monitor.timer"
echo ""
echo "Quick commands:"
