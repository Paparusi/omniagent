#!/usr/bin/env bash
# ClawDock - Docker helpers for OmniAgent
# Inspired by Simon Willison's "Running OmniAgent in Docker"
# https://til.simonwillison.net/llms/omniagent-docker
#
# Installation:
...
# Common paths to check for OmniAgent
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/omniagent"
  "${HOME}/workspace/omniagent"
  "${HOME}/projects/omniagent"
  "${HOME}/dev/omniagent"
  "${HOME}/code/omniagent"
  "${HOME}/src/omniagent"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found OmniAgent at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/omniagent"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OmniAgent not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/Paparusi/omniagent.git ~/omniagent"
    echo "  cd ~/omniagent && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OMNIAGENT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d omniagent-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart omniagent-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f omniagent-gateway
}
...
clawdock-workspace() {
  cd ~/.omniagent/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec omniagent-gateway \
    bash -c 'echo "alias omniagent=\"./omniagent.mjs\"" > /tmp/.bashrc_omniagent && bash --rcfile /tmp/.bashrc_omniagent'
}
...
clawdock-cli() {
  _clawdock_compose run --rm omniagent-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build omniagent-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OMNIAGENT_GATEWAY_TOKEN=$token" omniagent-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" omniagent-gateway \
    bash -c './omniagent.mjs config set gateway.remote.token "$TOKEN" && ./omniagent.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec omniagent-gateway \
    bash -c "./omniagent.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart omniagent-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm omniagent-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec omniagent-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'omniagent config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for OmniAgent${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (omniagent alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to omniagent project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.omniagent)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'omniagent channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'omniagent status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.omniagent.ai${_CLR_RESET}"
  echo ""
}
