...
ZIP=${1:?"Usage: $0 OmniAgent-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/omniagent/omniagent/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OmniAgent-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/Paparusi/omniagent/releases/download/v${VERSION}/"}
