 ...
 * `omniagent update`. Use it to verify the completion installation flow
...
\`omniagent update\`, \`omniagent doctor\`, and \`omniagent onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OmniAgent Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`omniagent completion --install\` later to enable.`));
    return;
  }
