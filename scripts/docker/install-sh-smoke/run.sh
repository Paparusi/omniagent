...
INSTALL_URL="${OMNIAGENT_INSTALL_URL:-https://omniagent.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OMNIAGENT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OMNIAGENT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="omniagent"
PACKAGE_NAME="${OMNIAGENT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OMNIAGENT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OMNIAGENT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OMNIAGENT_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
