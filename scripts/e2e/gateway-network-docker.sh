...
IMAGE_NAME="omniagent-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="omniagent-net-e2e-$$"
GW_NAME="omniagent-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OMNIAGENT_GATEWAY_TOKEN=$TOKEN" \
	  -e "OMNIAGENT_SKIP_CHANNELS=1" \
	  -e "OMNIAGENT_SKIP_GMAIL_WATCHER=1" \
	  -e "OMNIAGENT_SKIP_CRON=1" \
	  -e "OMNIAGENT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
