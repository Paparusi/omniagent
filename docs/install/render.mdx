...
Deploy OmniAgent on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/Paparusi/omniagent)
...
repository configures everything needed to run OmniAgent:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OMNIAGENT_STATE_DIR
        value: /data/.omniagent
      - key: OMNIAGENT_WORKSPACE_DIR
        value: /data/workspace
      - key: OMNIAGENT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/omniagent`.
...
If you use the original OmniAgent repository, Render will not auto-deploy your OmniAgent. To update it, run a manual Blueprint sync from the dashboard.
...
For OmniAgent, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OmniAgent host.
