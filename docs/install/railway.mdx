...
Deploy OmniAgent on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/omniagent` — Control UI
...
- `OMNIAGENT_STATE_DIR=/data/.omniagent` (recommended)
- `OMNIAGENT_WORKSPACE_DIR=/data/workspace` (recommended)
- `OMNIAGENT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OmniAgent state + workspace so you can migrate to another host without losing config or memory.
