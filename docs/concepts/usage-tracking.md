...
- `/usage cost` in chats: local cost summary aggregated from OmniAgent session logs.
- CLI: `omniagent status --usage` prints a full per-provider breakdown.
- CLI: `omniagent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
