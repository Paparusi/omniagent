...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.omniagent/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.omniagent/node.json` (node id + pairing token).
...
Path: `~/.omniagent/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.omniagent/exec-approvals.sock",
...
- Unix socket at `~/.omniagent/exec-approvals.sock` (0600).
