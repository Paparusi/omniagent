...
- Publish as `omniagent/plugin-sdk` (or export from core under `omniagent/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OmniAgentPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OmniAgentConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OmniAgentConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OmniAgentConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OmniAgentConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OmniAgentConfig): string;
  };
};
```
...
- Introduce `omniagent/plugin-sdk`.
- Add `api.runtime` to `OmniAgentPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `omniagentRuntime: ">=2026.2.0"`).
