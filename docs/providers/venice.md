---
summary: "Use Venice AI privacy-focused models in OmniAgent"
read_when:
  - You want privacy-focused inference in OmniAgent
...
```bash
omniagent onboard --auth-choice venice-api-key
```
...
```bash
omniagent chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OmniAgent shows all available Venice models. Pick based on your needs:
...
```bash
omniagent models set venice/claude-opus-45
omniagent models set venice/llama-3.3-70b
```

List all available models:

```bash
omniagent models list | grep venice
```

## Configure via `omniagent configure`

1. Run `omniagent configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OmniAgent automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
omniagent chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
omniagent chat --model venice/claude-opus-45

# Use uncensored model
omniagent chat --model venice/venice-uncensored

# Use vision model with image
omniagent chat --model venice/qwen3-vl-235b-a22b

# Use coding model
omniagent chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `omniagent models list` to see currently available models. Some models may be temporarily offline.
