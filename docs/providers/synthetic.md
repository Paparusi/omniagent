---
summary: "Use Synthetic's Anthropic-compatible API in OmniAgent"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OmniAgent registers it as the
...
Note: OmniAgent's Anthropic client appends `/v1` to the base URL, so use
