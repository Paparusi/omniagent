---
summary: "Use MiniMax M2.1 in OmniAgent"
read_when:
  - You want MiniMax models in OmniAgent
...
```bash
omniagent plugins enable minimax-portal-auth  # skip if already loaded.
omniagent gateway restart  # restart if gateway is already running
omniagent onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/Paparusi/omniagent/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `omniagent.json`:
...
- Use `omniagent models list` and `omniagent models set minimax/MiniMax-M2.1` to switch.
...
- Running `omniagent configure` and selecting **MiniMax M2.1**, or
