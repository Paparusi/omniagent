---
summary: "OmniAgent on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OmniAgent on a Raspberry Pi
  - Running OmniAgent on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OmniAgent on Raspberry Pi

## Goal

Run a persistent, always-on OmniAgent Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OmniAgent features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled omniagent
...
```bash
# Check logs
journalctl -u omniagent --no-pager -n 100

# Common fix: rebuild
cd ~/omniagent  # if using hackable install
npm run build
sudo systemctl restart omniagent
```
