---
summary: "OmniAgent on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OmniAgent on DigitalOcean
  - Looking for cheap VPS hosting for OmniAgent
title: "DigitalOcean"
---

# OmniAgent on DigitalOcean

## Goal

Run a persistent OmniAgent Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status omniagent-gateway.service

# View logs
journalctl --user -u omniagent-gateway.service -f
```
...
```bash
omniagent config set gateway.bind tailnet
omniagent gateway restart
```
...
```bash
omniagent channels login whatsapp
# Scan QR code
```
...
- `~/.omniagent/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u omniagent --no-pager -n 50
```
