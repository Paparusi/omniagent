...
service via the external `omniagent` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OMNIAGENT_PROFILE`; legacy `com.omniagent.*` is supported).
