...
read_when:
  - You want to place an outbound voice call from OmniAgent
...
Voice calls for OmniAgent via a plugin. Supports outbound notifications and
...
- Use `omniagent voicecall ...` or the `voice_call` tool
...
```bash
omniagent plugins install ./extensions/voice-call
