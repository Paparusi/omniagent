---
summary: "Chrome extension: let OmniAgent drive your existing Chrome tab"
read_when:
...
The OmniAgent Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate omniagent-managed Chrome profile.
...
OmniAgent then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OmniAgent release (npm package) as static files. There is no separate “build” step.

After upgrading OmniAgent:

- Re-run `omniagent browser extension install` to refresh the installed files under your OmniAgent state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OmniAgent ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `OMNIAGENT_GATEWAY_TOKEN`)
...
- Open the tab you want OmniAgent to control.
...
- `ON`: attached; OmniAgent can drive that tab.
...
Debugging: `omniagent sandbox explain`
...
`omniagent browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `omniagent browser extension install` first to copy the extension to a stable location under your OmniAgent state directory.
...
- **This is not isolated** like the dedicated omniagent-managed profile.
