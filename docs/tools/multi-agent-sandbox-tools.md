...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `omniagent sandbox explain`.
...
Legacy `agent.*` configs are migrated by `omniagent doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=omniagent-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OMNIAGENT_STATE_DIR:-$HOME/.omniagent}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
