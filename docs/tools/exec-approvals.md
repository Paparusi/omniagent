...
- **gateway host** → `omniagent` process on the gateway machine
...
`~/.omniagent/exec-approvals.json` directly.

CLI: `omniagent approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
