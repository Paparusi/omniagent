---
summary: "Agent tool surface for OmniAgent (browser, canvas, nodes, message, cron) replacing legacy `omniagent-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `omniagent-*` skills
title: "Tools"
---

# Tools (OmniAgent)

OmniAgent exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `omniagent-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `omniagent.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OmniAgent logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:omniagent`: all built-in OmniAgent tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`omniagent node run`).
...
OmniAgent tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `omniagent configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OmniAgent-managed browser.
...
- Quick smoke: `omniagent nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `omniagent gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OmniAgent clamps `tools.sessions.visibility` to `tree`.
