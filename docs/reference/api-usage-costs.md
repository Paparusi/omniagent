...
OmniAgent features that can generate provider usage or paid API calls.
...
- `omniagent status --usage` and `omniagent channels list` show provider **usage windows**
...
OmniAgent can pick up credentials from:
...
`omniagent models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
