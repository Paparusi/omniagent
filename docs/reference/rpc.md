...
OmniAgent integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OmniAgent owns lifecycle when `channels.signal.autoStart=true`.
...
- OmniAgent spawns `imsg rpc` as a child process (legacy iMessage integration).
