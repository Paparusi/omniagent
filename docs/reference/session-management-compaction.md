...
This document explains how OmniAgent manages sessions end-to-end:
...
OmniAgent is designed around a single **Gateway process** that owns session state.
...
OmniAgent persists sessions in two layers:
...
- Transcripts: `~/.omniagent/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

OmniAgent resolves these via `src/config/sessions.ts`.
...
OmniAgent intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OmniAgent consumes the events, but Pi decides when to compact).
...
OmniAgent also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OmniAgent bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OmniAgent leaves it alone.
...
- `omniagent status` (CLI)
- `omniagent sessions` / `sessions --json`
...
OmniAgent supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OmniAgent strips/suppresses this in the delivery layer.

As of `2026.1.10`, OmniAgent also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OmniAgent uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OmniAgent’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `omniagent status`.
