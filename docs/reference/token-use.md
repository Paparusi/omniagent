---
summary: "How OmniAgent builds prompt context and reports token usage + costs"
read_when:
...
OmniAgent tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OmniAgent assembles its own system prompt on every run. It includes:
...
For images, OmniAgent downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OmniAgent session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `omniagent status --usage` and `omniagent channels list` show
...
`cacheWrite`. If pricing is missing, OmniAgent shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OmniAgent can
...
Anthropic's 1M context window is currently beta-gated. OmniAgent can inject the
