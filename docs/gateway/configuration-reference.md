...
description: "Complete field-by-field reference for ~/.omniagent/omniagent.json"
---

# Configuration Reference

Every field available in `~/.omniagent/omniagent.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OmniAgent uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `omniagent doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["omniagent-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `omniagent plugins install @omniagent/mattermost`.
...
OmniAgent spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@omniagent"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `omniagent.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OmniAgent auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OmniAgent assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "omniagent-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.omniagent/sandboxes`
...
noVNC observer access uses VNC auth by default and OmniAgent emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `omniagent-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.omniagent/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@omniagent"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:omniagent`   | All built-in tools (excludes provider plugins)                                           |
...
OmniAgent uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.omniagent/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OMNIAGENT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `omniagent onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `omniagent onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `omniagent onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `omniagent onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `omniagent onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `omniagent onboard --auth-choice minimax-api`.
...
- Loaded from `~/.omniagent/extensions`, `<workspace>/.omniagent/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OMNIAGENT_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OMNIAGENT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.omniagent-dev` + port `19001`), `--profile <name>` (uses `~/.omniagent-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.omniagent/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-omniagent-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OMNIAGENT_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OMNIAGENT_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__omniagent__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__omniagent__/a2ui/`
...
- Also serves A2UI at `/__omniagent__/a2ui/`.
...
- Hostname defaults to `omniagent`. Override with `OMNIAGENT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.omniagent/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `omniagent dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.omniagent/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OMNIAGENT_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.omniagent/credentials/oauth.json`.
...
- Default log file: `/tmp/omniagent/omniagent-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `omniagent doctor --fix` can strip unknown keys).
