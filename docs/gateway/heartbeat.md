...
- During heartbeat runs, OmniAgent treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `omniagent sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OmniAgent skips the heartbeat run entirely (no model call).
...
```bash
omniagent system event --text "Check for urgent follow-ups" --mode now
```
