...
read_when:
  - Running OmniAgent behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OmniAgent
...
- You run OmniAgent behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OmniAgent checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OmniAgent extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://omniagent.example.com
    to: http://omniagent-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
omniagent.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy omniagent:18789 {
...
location / {
    ...
    proxy_pass http://omniagent:18789;
...
`omniagent security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OmniAgent config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `omniagent security audit` and review findings
