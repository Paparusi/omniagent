...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OmniAgent
...
Running OmniAgent via Node with `tsx` fails at startup with:

```
[omniagent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `omniagent status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch omniagent.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node omniagent.mjs status` works on Node 25.
