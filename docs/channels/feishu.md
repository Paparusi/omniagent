...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OmniAgent to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
omniagent plugins install @omniagent/feishu
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/feishu
```
...
If you just installed OmniAgent, run the wizard:
...
2. Configuring app credentials in OmniAgent
...
1. You already ran `omniagent channels add` for Feishu
2. The gateway is running (`omniagent gateway status`)
...
```bash
omniagent pairing approve feishu <CODE>
```
...
2. Run `omniagent logs --follow` and look for `chat_id`
...
2. Run `omniagent logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `omniagent gateway status`  | Show gateway status           |
| `omniagent gateway install` | Install/start gateway service |
| `omniagent gateway stop`    | Stop gateway service          |
| `omniagent gateway restart` | Restart gateway service       |
| `omniagent logs --follow`   | Tail gateway logs             |
...
4. Check logs: `omniagent logs --follow`
...
5. Ensure the gateway is running: `omniagent gateway status`
6. Check logs: `omniagent logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.omniagent/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.omniagent/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
