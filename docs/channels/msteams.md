...
```bash
omniagent plugins install @omniagent/msteams
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OmniAgent will offer the local install path automatically.
...
3. Configure OmniAgent with those credentials.
...
- Talk to OmniAgent via Teams DMs, group chats, or channels.
...
- On startup, OmniAgent resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.omniagent/omniagent.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OmniAgent, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `omniagent-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `omniagent plugins install @omniagent/msteams`
   - From a local checkout: `omniagent plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OmniAgent in Teams", "full": "OmniAgent in Teams" },
...
OmniAgent handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OmniAgent only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OmniAgentShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OmniAgent sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `omniagent message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.omniagent/msteams-polls.json`.
...
```bash
omniagent message send --channel msteams \
...
omniagent message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
omniagent message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
omniagent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
omniagent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
