...
Matrix is an open, decentralized messaging protocol. OmniAgent connects as a Matrix **user**
...
```bash
omniagent plugins install @omniagent/matrix
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `omniagent plugins install @omniagent/matrix`
   - From a local checkout: `omniagent plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OmniAgent calls the same
     login endpoint, stores the access token in `~/.omniagent/credentials/matrix/credentials.json`,
...
- On first connection, OmniAgent requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OmniAgent logs a warning.
...
`~/.omniagent/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `omniagent pairing list matrix`
  - `omniagent pairing approve matrix <CODE>`
...
- On startup, OmniAgent resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
