---
summary: "Mattermost bot setup and OmniAgent config"
read_when:
...
```bash
omniagent plugins install @omniagent/mattermost
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/mattermost
```
...
4. Configure OmniAgent and start the gateway.
...
- Approve via:
  - `omniagent pairing list mattermost`
  - `omniagent pairing approve mattermost <CODE>`
...
Use these target formats with `omniagent message send` or cron/webhooks:
