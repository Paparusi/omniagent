...
Tlon is a decentralized messenger built on Urbit. OmniAgent connects to your Urbit ship and can
...
```bash
omniagent plugins install @omniagent/tlon
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/tlon
```
...
By default, OmniAgent blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `omniagent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OmniAgent replies in-thread.
