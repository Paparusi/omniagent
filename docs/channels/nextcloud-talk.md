...
```bash
omniagent plugins install @omniagent/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OmniAgent" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `omniagent pairing list nextcloud-talk`
  - `omniagent pairing approve nextcloud-talk <CODE>`
