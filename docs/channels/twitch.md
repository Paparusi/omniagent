...
read_when:
  - Setting up Twitch chat integration for OmniAgent
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OmniAgent connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
omniagent plugins install @omniagent/twitch
```

Local checkout (when running from a git repo):

```bash
omniagent plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OMNIAGENT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "omniagent", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OMNIAGENT_TWITCH_ACCESS_TOKEN env var)
...
```bash
OMNIAGENT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
