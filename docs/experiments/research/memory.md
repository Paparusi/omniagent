...
read_when:
  - Designing workspace memory (~/.omniagent/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OmniAgent integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.omniagent/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.omniagent/workspace` as canonical human-readable memory.
...
```
~/.omniagent/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OmniAgent**, but keep a separable core library.

### Why integrate into OmniAgent?

- OmniAgent already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `omniagent memory recall "…" --k 25 --since 30d`
  - `omniagent memory reflect --since 7d`
...
Pragmatic take for `~/.omniagent/workspace`:
