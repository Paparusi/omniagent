...
你可以在 `~/.omniagent/omniagent.json` 中覆盖此设置：
...
所有日志配置都在 `~/.omniagent/omniagent.json` 的 `logging` 下。
...
- OmniAgent 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `omniagent plugins enable diagnostics-otel` 启用插件。
...
- `omniagent.tokens`（计数器，属性：`omniagent.token`、`omniagent.channel`、`omniagent.provider`、`omniagent.model`）
- `omniagent.cost.usd`（计数器，属性：`omniagent.channel`、`omniagent.provider`、`omniagent.model`）
- `omniagent.run.duration_ms`（直方图，属性：`omniagent.channel`、`omniagent.provider`、`omniagent.model`）
- `omniagent.context.tokens`（直方图，属性：`omniagent.context`、`omniagent.channel`、`omniagent.provider`、`omniagent.model`）

消息流：

- `omniagent.webhook.received`（计数器，属性：`omniagent.channel`、`omniagent.webhook`）
- `omniagent.webhook.error`（计数器，属性：`omniagent.channel`、`omniagent.webhook`）
- `omniagent.webhook.duration_ms`（直方图，属性：`omniagent.channel`、`omniagent.webhook`）
- `omniagent.message.queued`（计数器，属性：`omniagent.channel`、`omniagent.source`）
- `omniagent.message.processed`（计数器，属性：`omniagent.channel`、`omniagent.outcome`）
- `omniagent.message.duration_ms`（直方图，属性：`omniagent.channel`、`omniagent.outcome`）

队列 + 会话：

- `omniagent.queue.lane.enqueue`（计数器，属性：`omniagent.lane`）
- `omniagent.queue.lane.dequeue`（计数器，属性：`omniagent.lane`）
- `omniagent.queue.depth`（直方图，属性：`omniagent.lane` 或 `omniagent.channel=heartbeat`）
- `omniagent.queue.wait_ms`（直方图，属性：`omniagent.lane`）
- `omniagent.session.state`（计数器，属性：`omniagent.state`、`omniagent.reason`）
- `omniagent.session.stuck`（计数器，属性：`omniagent.state`）
- `omniagent.session.stuck_age_ms`（直方图，属性：`omniagent.state`）
- `omniagent.run.attempt`（计数器，属性：`omniagent.attempt`）
...
- `omniagent.webhook.error`
  - `omniagent.channel`、`omniagent.webhook`、`omniagent.chatId`、`omniagent.error`
- `omniagent.message.processed`
  - `omniagent.channel`、`omniagent.outcome`、`omniagent.chatId`、`omniagent.messageId`、`omniagent.sessionKey`、`omniagent.sessionId`、`omniagent.reason`
- `omniagent.session.stuck`
  - `omniagent.state`、`omniagent.ageMs`、`omniagent.queueDepth`、`omniagent.sessionKey`、`omniagent.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `omniagent doctor`。
