---
summary: "CLI reference for `omniagent security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `omniagent.browserConfigEpoch`) and recommends `omniagent sandbox recreate --browser --all`.
...
```bash
omniagent security audit --json | jq '.summary'
omniagent security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
omniagent security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
