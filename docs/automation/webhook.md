...
read_when:
  ...
  - Wiring external systems into OmniAgent
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OmniAgent config directory (typically `~/.omniagent/hooks/transforms`).
...
- `omniagent webhooks gmail setup` writes `hooks.gmail` config for `omniagent webhooks gmail run`.
