...
- Jobs persist under `~/.omniagent/cron/` so restarts don’t lose schedules.
...
omniagent cron list
omniagent cron run <job-id>
omniagent cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.omniagent/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `omniagent cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OmniAgent applies a
...
If `delivery` is omitted for isolated jobs, OmniAgent defaults to `announce`.
...
- Job store: `~/.omniagent/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.omniagent/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.omniagent/cron/jobs.json",
...
omniagent cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
omniagent cron edit <jobId> --agent ops
omniagent cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
omniagent cron run <jobId>
omniagent cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
omniagent cron edit <jobId> \
...
```bash
omniagent cron edit <jobId> --exact
```
...
```bash
omniagent system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`omniagent system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OMNIAGENT_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OmniAgent applies exponential retry backoff for recurring jobs after consecutive errors:
