...
```bash
omniagent tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `omniagent tui --deliver`
...
- Check the Gateway logs: `omniagent logs --follow`.
- Confirm the agent can run: `omniagent status` and `omniagent models status`.
...
- No agents in picker: check `omniagent agents list` and your routing config.
