...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/omniagent`)
...
If the page fails to load, start the Gateway first: `omniagent gateway`.
...
you revoke it with `omniagent devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.omniagent/omniagent.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OmniAgent
...
OmniAgent **blocks** Control UI connections without device identity.
