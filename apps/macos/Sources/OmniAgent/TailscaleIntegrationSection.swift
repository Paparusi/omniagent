...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.omniagent/omniagent.json. Prefer OMNIAGENT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.omniagent/omniagent.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.omniagent/omniagent.json. Restart the gateway to apply."
        }
