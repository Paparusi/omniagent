...
private let deepLinkLogger = Logger(subsystem: "ai.omniagent", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "OmniAgent is paused", message: "Unpause OmniAgent to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run OmniAgent agent?", message: body) else { return }
        }
