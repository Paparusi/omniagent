...
enum OmniAgentLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OmniAgentOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OmniAgentFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OmniAgentLogging.bootstrapIfNeeded()
        let label = OmniAgentLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OmniAgentOSLogHandler: LogHandler {
...
struct OmniAgentFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OmniAgentLogging.parseLabel(self.label)
