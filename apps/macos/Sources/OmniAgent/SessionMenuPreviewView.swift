...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.omniagent", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OmniAgentSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OmniAgentSessionPreviewEntry,
    ...
    private static func cache(payload: OmniAgentSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OmniAgentChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OmniAgentKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OmniAgentKit.AnyCodable]) -> [OmniAgentChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OmniAgentChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OmniAgentChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OmniAgentChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OmniAgentChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OmniAgentChatMessage) -> String? {
