...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OmniAgentProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.omniagent", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OmniAgentKit.AnyCodable]()) {
                $0[$1.key] = OmniAgentKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OmniAgentProtocol.AnyCodable?) -> [String: OmniAgentProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OmniAgentProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OmniAgentKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OmniAgentProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("omniagent.control.heartbeat")
    static let controlAgentEvent = Notification.Name("omniagent.control.agent")
}
