...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.omniagent", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OmniAgentPeekabooBridgeServices?
    private static var omniagentSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OmniAgent", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OmniAgentPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.omniagentSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.omniagentSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OmniAgentPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.omniagent.peekaboo")
