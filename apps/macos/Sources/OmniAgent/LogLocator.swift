...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OMNIAGENT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/omniagent")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("omniagent-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("omniagent-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/omniagent/ (rolling or stdout), or nil if none exist.
