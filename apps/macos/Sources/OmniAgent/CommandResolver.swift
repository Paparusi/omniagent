...
enum CommandResolver {
    private static let projectRootDefaultsKey = "omniagent.gatewayProjectRootPath"
    private static let helperName = "omniagent"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let omniagentEntry = root.appendingPathComponent("omniagent.mjs").path
        if FileManager().isReadableFile(atPath: omniagentEntry) { return omniagentEntry }
        let binEntry = root.appendingPathComponent("bin/omniagent.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/omniagent")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let omniagentPaths = self.omniagentManagedPaths(home: home)
        if !omniagentPaths.isEmpty {
            extras.insert(contentsOf: omniagentPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + omniagentPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func omniagentManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".omniagent"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func omniagentExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOmniAgentExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("omniagent.mjs").path,
            root.appendingPathComponent("bin/omniagent.js").path,
        ]
    ...
    static func hasAnyOmniAgentInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.omniagentExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func omniagentNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let omniagentPath = self.projectOmniAgentExecutable(projectRoot: root) {
                return [omniagentPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "omniagent", subcommand] + extraArgs
            }
            if let omniagentPath = self.omniagentExecutable(searchPaths: searchPaths) {
                return [omniagentPath, subcommand] + extraArgs
            }

            let missingEntry = """
            omniagent entrypoint missing (looked for dist/index.js or omniagent.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real omniagent CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/omniagent"
        ...
        \(cliSection)
        \(projectSection)
        if command -v omniagent >/dev/null 2>&1; then
          CLI="$(command -v omniagent)"
          omniagent \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/omniagent.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/omniagent.mjs"
            node "$PRJ/omniagent.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/omniagent.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/omniagent.js"
            node "$PRJ/bin/omniagent.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent omniagent"
          pnpm --silent omniagent \(quotedArgs);
        else
          echo "omniagent CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OmniAgentConfigFile.loadDict()
