...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (OmniAgent token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: OmniAgentOAuthStore.AnthropicOAuthStatus = OmniAgentOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.omniagent", category: "anthropic-oauth")
...
enum OmniAgentOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let omniagentOAuthDirEnv = "OMNIAGENT_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "OmniAgent OAuth token file not found"
            case .unreadableFile: "OmniAgent OAuth token file not readable"
            case .invalidJSON: "OmniAgent OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in OmniAgent OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "OmniAgent OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.omniagentOAuthDirEnv]?
