...
private let webChatSwiftLogger = Logger(subsystem: "ai.omniagent", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OmniAgentChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OmniAgentChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OmniAgentChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OmniAgentChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OmniAgentGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    OmniAgentChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OmniAgentAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OmniAgentChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OmniAgentChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OmniAgentChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OmniAgentChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OmniAgentChatView>) -> NSViewController
    {
