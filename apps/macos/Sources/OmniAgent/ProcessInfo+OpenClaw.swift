...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.omniagent.mac`) even if the shipped
    ...
    {
        if environment["OMNIAGENT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "omniagent.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "omniagent.nixMode") { return true }
