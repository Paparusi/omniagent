...
enum DebugActions {
    private static let verboseDefaultsKey = "omniagent.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "omniagent.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OmniAgentPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OmniAgent", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OmniAgentPaths.configURL
        guard
