...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OmniAgentOAuthStore.AnthropicOAuthStatus = OmniAgentOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OmniAgentOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OmniAgentOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OmniAgentOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try OmniAgentOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. OmniAgent can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OmniAgentOAuthStore.AnthropicOAuthStatus,
