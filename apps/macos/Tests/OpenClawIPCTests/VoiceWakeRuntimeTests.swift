...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["omniagent"]
        let text = "hey omniagent"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey omniagent do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["omniagent"], minPostTriggerGap: 0.3)
