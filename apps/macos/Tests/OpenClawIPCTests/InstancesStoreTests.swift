...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OmniAgentProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OmniAgentProtocol.AnyCodable] = [
            "presence": .init([OmniAgentProtocol.AnyCodable(entry)]),
        ]
        let payload = OmniAgentProtocol.AnyCodable(payloadMap)
