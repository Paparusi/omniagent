...
import Foundation
import Testing
@testable import OmniAgentChatUI
...
private final class TestChatTransport: @unchecked Sendable, OmniAgentChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OmniAgentChatHistoryPayload]
    private let sessionsResponses: [OmniAgentChatSessionsListResponse]

    private let stream: AsyncStream<OmniAgentChatTransportEvent>
    private let continuation: AsyncStream<OmniAgentChatTransportEvent>.Continuation

    init(
        historyResponses: [OmniAgentChatHistoryPayload],
        sessionsResponses: [OmniAgentChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OmniAgentChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OmniAgentChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OmniAgentChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OmniAgentChatAttachmentPayload]) async throws -> OmniAgentChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OmniAgentChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OmniAgentChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OmniAgentChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OmniAgentChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OmniAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OmniAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OmniAgentChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OmniAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OmniAgentChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OmniAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OmniAgentChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
