...
@MainActor
struct OmniAgentChatComposer: View {
    @Bindable var viewModel: OmniAgentChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OmniAgentChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OmniAgentChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OmniAgentPendingAttachment.id)
                { (att: OmniAgentPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OmniAgentPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(OmniAgentChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OmniAgentChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OmniAgentChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OmniAgentChatTheme.subtleCard)
        .clipShape(Capsule())
    }
