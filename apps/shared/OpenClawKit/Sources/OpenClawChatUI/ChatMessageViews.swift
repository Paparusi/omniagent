...
struct ChatMessageBubble: View {
    let message: OmniAgentChatMessage
    let style: OmniAgentChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OmniAgentChatTheme.userText : OmniAgentChatTheme.assistantText
    ...
    private var inlineAttachments: [OmniAgentChatMessageContent] {
    ...
    private var toolCalls: [OmniAgentChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OmniAgentChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OmniAgentChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OmniAgentChatTheme.onboardingAssistantBubble
        }
        return OmniAgentChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OmniAgentChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OmniAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OmniAgentChatTheme.userText : OmniAgentChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OmniAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OmniAgentChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OmniAgentChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OmniAgentChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OmniAgentChatTheme.assistantText)
            }
        }
    }
}
