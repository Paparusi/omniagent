...
private let chatUILogger = Logger(subsystem: "ai.omniagent", category: "OmniAgentChatUI")

@MainActor
@Observable
public final class OmniAgentChatViewModel {
    public private(set) var messages: [OmniAgentChatMessage] = []
    ...
    public var attachments: [OmniAgentPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OmniAgentChatPendingToolCall] = []
    public private(set) var sessions: [OmniAgentChatSessionEntry] = []
    private let transport: any OmniAgentChatTransport
    ...
    private var pendingToolCallsById: [String: OmniAgentChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OmniAgentChatTransport) {
    ...
    public var sessionChoices: [OmniAgentChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [OmniAgentChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OmniAgentPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OmniAgentChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OmniAgentChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OmniAgentChatMessage) -> OmniAgentChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OmniAgentChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OmniAgentChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OmniAgentChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OmniAgentChatMessage],
        incoming: [OmniAgentChatMessage]) -> [OmniAgentChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OmniAgentChatMessage]) -> [OmniAgentChatMessage] {
        var result: [OmniAgentChatMessage] = []
    ...
    private static func dedupeKey(for message: OmniAgentChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OmniAgentChatMessageContent] = [
            OmniAgentChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OmniAgentChatAttachmentPayload in
            OmniAgentChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OmniAgentChatSessionEntry {
        OmniAgentChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OmniAgentChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OmniAgentChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OmniAgentAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OmniAgentChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OmniAgentPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
