...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.omniagent.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OmniAgent", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OmniAgent gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OmniAgent.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OmniAgentShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OmniAgent is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OmniAgent Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("omniagent-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
