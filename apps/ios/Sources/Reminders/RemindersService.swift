...
final class RemindersService: RemindersServicing {
    func list(params: OmniAgentRemindersListParams) async throws -> OmniAgentRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OmniAgentReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OmniAgentReminderPayload(
        ...
        return OmniAgentRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OmniAgentRemindersAddParams) async throws -> OmniAgentRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OmniAgentReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OmniAgentRemindersAddPayload(reminder: payload)
    }
