...
struct IOSGatewayChatTransport: OmniAgentChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.omniagent", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OmniAgentChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OmniAgentChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OmniAgentChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OmniAgentChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OmniAgentChatAttachmentPayload]) async throws -> OmniAgentChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OmniAgentChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OmniAgentChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OmniAgentGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OmniAgentGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OmniAgentChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OmniAgentAgentEventPayload.self)
                        {
