...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OmniAgentNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OmniAgentNetworkStatusPayload {
        let status: OmniAgentNetworkPathStatus = switch path.status {
        ...
        var interfaces: [OmniAgentNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return OmniAgentNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> OmniAgentNetworkStatusPayload {
        OmniAgentNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
