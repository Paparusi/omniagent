...
final class MotionService: MotionServicing {
    func activities(params: OmniAgentMotionActivityParams) async throws -> OmniAgentMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OmniAgentMotionActivityEntry], Error>) in
        ...
        return OmniAgentMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OmniAgentPedometerParams) async throws -> OmniAgentPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OmniAgentPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OmniAgentPedometerPayload(
