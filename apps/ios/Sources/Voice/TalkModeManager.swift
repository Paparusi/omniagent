...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OMNIAGENT_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OmniAgentTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OmniAgentTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OmniAgentTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OmniAgentTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OmniAgentTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OmniAgentTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OmniAgentTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OmniAgentTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OmniAgentTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OmniAgentTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: OmniAgentAgentEventPayload.self) else {
                continue
            }
