...
`omniagent.invoke --each`) without adding new OmniAgent code per workflow.
...
This extension depends on OmniAgent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OmniAgent extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.omniagent/extensions` as a standalone plugin directory.
