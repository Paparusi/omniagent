...
import type { OmniAgentPluginApi, OmniAgentPluginService } from "omniagent/plugin-sdk";
...
function normalizeDenyList(cfg: OmniAgentPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OmniAgentPluginApi["config"]): string[] {
...
export default function register(api: OmniAgentPluginApi) {
  ...
  const timerService: OmniAgentPluginService = {
