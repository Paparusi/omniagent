...
function resolveOmniAgentRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OMNIAGENT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OMNIAGENT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOmniAgentRoot(), "dist", "extensionAPI.js");
