...
import { approveDevicePairing, listDevicePairing } from "omniagent/plugin-sdk";
...
function resolveGatewayPort(cfg: OmniAgentPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OMNIAGENT_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: OmniAgentPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OmniAgentPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OmniAgent iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OmniAgent iOS app:",
            "",
            "```",
            qrAscii,
            "```",
