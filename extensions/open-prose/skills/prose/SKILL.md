...
metadata: { "omniagent": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OmniAgent `sessions_spawn`
- **File I/O** == OmniAgent `read`/`write`
- **Remote fetch** == OmniAgent `web_fetch` (or `exec` with curl when POST is required)
