...
import type { ReplyPayload, OmniAgentConfig } from "omniagent/plugin-sdk";
import { createReplyPrefixOptions } from "omniagent/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // OmniAgentConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as OmniAgentConfig;
