...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: OMNIAGENT_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: OmniAgentConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var OMNIAGENT_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.OMNIAGENT_TWITCH_ACCESS_TOKEN?.trim();
