/**
 * Twitch channel plugin for OmniAgent.
 *
...
import { buildChannelConfigSchema } from "omniagent/plugin-sdk";
 ...
 * for OmniAgent. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OmniAgentConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OmniAgentConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OmniAgentConfig): boolean => {
