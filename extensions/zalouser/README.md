...
OmniAgent extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
omniagent channels login --channel zalouser
...
```bash
omniagent message send --channel zalouser --target <threadId> --message "Hello from OmniAgent!"
```
...
```bash
omniagent channels login --channel zalouser              # Login via QR
omniagent channels login --channel zalouser --account work
omniagent channels status --probe
omniagent channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
omniagent directory self --channel zalouser
omniagent directory peers list --channel zalouser --query "name"
omniagent directory groups list --channel zalouser --query "work"
omniagent directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
omniagent message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
omniagent message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `omniagent channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
omniagent channels login --channel zalouser --account work
omniagent message send --channel zalouser --account work --target <id> --message "Hello"
