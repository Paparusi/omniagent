...
export function getForgeClient(cfg: A2AConfig): any {
  ...
  if (!cfg.agentforge?.apiKey) {
    throw new Error("AgentForge API key required — set agentforge.apiKey in plugin config");
  }
...
export async function getPassBoxClient(cfg: A2AConfig): Promise<any> {
  ...
  if (!cfg.passbox?.token) {
    throw new Error("PassBox token required — set passbox.token in plugin config");
  }
...
export function getApayClient(cfg: A2AConfig): ApayHttpClient {
  ...
  if (!cfg.apay?.apiKey) {
    throw new Error("APay API key required — set apay.apiKey in plugin config");
  }
