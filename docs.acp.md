# OmniAgent ACP Bridge

This document describes how the OmniAgent ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`omniagent acp` exposes an ACP agent over stdio and forwards prompts to a running
OmniAgent Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `omniagent acp` and speaks ACP messages over stdio.
...
`omniagent acp` resolves the Gateway URL and auth from CLI flags or config:
