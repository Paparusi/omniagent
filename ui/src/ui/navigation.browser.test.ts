...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/omniagent/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/omniagent");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/omniagent/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OMNIAGENT_CONTROL_UI_BASE_PATH__ = "/omniagent";
    const app = mountApp("/omniagent/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/omniagent");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/omniagent/sessions");
  });
