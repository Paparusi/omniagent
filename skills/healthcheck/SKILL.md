---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OmniAgent deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OmniAgent cron scheduling for periodic checks, or version status checks on a machine running OmniAgent (laptop, workstation, Pi, VPS).
---

# OmniAgent Host Hardening

## Overview

Assess and harden the host running OmniAgent, then align it to a user-defined risk tolerance without breaking access. Use OmniAgent security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OmniAgent changes the host firewall, SSH, or OS updates; it does not.
...
5. OmniAgent gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OmniAgent can access sensitive data.
...
### 2) Run OmniAgent security audits (read-only)

As part of the default read-only checks, run `omniagent security audit --deep`. Only offer alternatives if the user requests them:

1. `omniagent security audit` (faster, non-probing)
2. `omniagent security audit --json` (structured output)

Offer to apply OmniAgent safe defaults (numbered):

1. `omniagent security audit --fix`

Be explicit that `--fix` only tightens OmniAgent defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OmniAgent version/update status (read-only)

As part of the default read-only checks, run `omniagent update status`.
...
- Credential hygiene notes (location of OmniAgent creds, prefer disk encryption)
...
- OmniAgent security audit (re-run)
...
After OmniAgent install or first hardening pass, run at least one baseline audit and version check:

- `omniagent security audit`
- `omniagent security audit --deep`
- `omniagent update status`

Ongoing monitoring is recommended. Use the OmniAgent cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `omniagent cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `omniagent update status`
...
Before creating, `omniagent cron list` and match on exact `name`. If found, `omniagent cron edit <id> ...`.
If not found, `omniagent cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `omniagent update status` (preferred for source checkouts and channels)
2. `npm view omniagent version` (published npm version)

## OmniAgent command accuracy

Use only supported commands and flags:

- `omniagent security audit [--deep] [--fix] [--json]`
- `omniagent status` / `omniagent status --deep`
- `omniagent health --json`
- `omniagent update status`
- `omniagent cron add|list|runs|run`

Do not invent CLI flags or imply OmniAgent enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OmniAgent compaction:
