...
process.env.OMNIAGENT_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime OmniAgent modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: OmniAgentConfig) => {
    ...
    resolveAccount: (cfg: OmniAgentConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: OmniAgentConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
