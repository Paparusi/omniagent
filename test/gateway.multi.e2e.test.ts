...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `omniagent-e2e-${name}-`));
  const configDir = path.join(homeDir, ".omniagent");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OMNIAGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-omniagent-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-omniagent-token": gwB.hookToken },
        ),
      ]);
