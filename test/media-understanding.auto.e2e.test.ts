...
import type { OmniAgentConfig } from "../src/config/config.js";
import { resolvePreferredOmniAgentTmpDir } from "../src/infra/tmp-omniagent-dir.js";
...
const makeTempDir = async (prefix: string) => {
  const baseDir = resolvePreferredOmniAgentTmpDir();
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("omniagent-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "omniagent-bin-sherpa-");
      const modelDir = await createTrackedTempDir(tempPaths, "omniagent-sherpa-model-");
      ...
      const cfg: OmniAgentConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "omniagent-bin-whispercpp-");
      const modelDir = await createTrackedTempDir(tempPaths, "omniagent-whispercpp-model-");
  ...
  it("uses gemini CLI for images when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "omniagent-bin-gemini-");
      ...
      const cfg: OmniAgentConfig = { tools: { media: { image: {} } } };
  ...
  it("skips auto-detect when no supported binaries are available", async () => {
    await withEnvSnapshot(async () => {
      const emptyBinDir = await createTrackedTempDir(tempPaths, "omniagent-bin-empty-");
