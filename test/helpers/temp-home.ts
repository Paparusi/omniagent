...
type EnvSnapshot = {
  ...
  omniagentHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    omniagentHome: process.env.OMNIAGENT_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OMNIAGENT_HOME", snapshot.omniagentHome);
  restoreKey("OMNIAGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OMNIAGENT_HOME.
  delete process.env.OMNIAGENT_HOME;
  process.env.OMNIAGENT_STATE_DIR = path.join(base, ".omniagent");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "omniagent-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".omniagent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
