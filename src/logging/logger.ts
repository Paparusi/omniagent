...
export const DEFAULT_LOG_DIR = resolvePreferredOmniAgentTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "omniagent.log"); // legacy single-file path

const LOG_PREFIX = "omniagent";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OmniAgentConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => OmniAgentConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.OMNIAGENT_TEST_FILE_LOG !== "1" ? "silent" : "info";
