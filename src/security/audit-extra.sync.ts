...
function summarizeGroupPolicy(cfg: OmniAgentConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: OmniAgentConfig): boolean {
...
function collectModels(cfg: OmniAgentConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: OmniAgentConfig): Set<string> {
  const baseCfg: OmniAgentConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: OmniAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OmniAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OmniAgentConfig): boolean {
...
function isBrowserEnabled(cfg: OmniAgentConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OmniAgentConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OMNIAGENT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("omniagent security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer OMNIAGENT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const omniagentGatewayToken =
    typeof env.OMNIAGENT_GATEWAY_TOKEN === "string" && env.OMNIAGENT_GATEWAY_TOKEN.trim()
      ? env.OMNIAGENT_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-omniagent-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: omniagent-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: OmniAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
