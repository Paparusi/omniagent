...
import { discoverOmniAgentPlugins } from "../../plugins/discovery.js";
import type { OmniAgentPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, OmniAgentPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["OMNIAGENT_PLUGIN_CATALOG_PATHS", "OMNIAGENT_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<OmniAgentPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: OmniAgentPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: OmniAgentPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOmniAgentPlugins({ workspaceDir: options.workspaceDir });
