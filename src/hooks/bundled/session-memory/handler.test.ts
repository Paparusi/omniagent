...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => OmniAgentConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("omniagent-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): OmniAgentConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
