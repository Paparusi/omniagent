...
homepage: https://docs.omniagent.ai/automation/hooks#command-logger
metadata:
  {
    "omniagent":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.omniagent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.omniagent/logs/commands.log ~/.omniagent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/omniagent`:
   ```
   /home/username/.omniagent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
