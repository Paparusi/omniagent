...
This directory contains hooks that ship with OmniAgent. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.omniagent/omniagent.json`:
...
2. Restart gateway: `pkill -9 -f 'omniagent.*gateway' && pnpm omniagent gateway`
3. Enable the hook: `omniagent hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.omniagent.ai/automation/hooks
