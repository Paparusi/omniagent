...
describe("resolveGatewayConnection", () => {
  ...
  it("uses OMNIAGENT_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ OMNIAGENT_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers OMNIAGENT_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ OMNIAGENT_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
