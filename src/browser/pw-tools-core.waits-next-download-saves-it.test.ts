...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOmniAgentTmpDir: vi.fn(() => "/tmp/omniagent"),
}));
vi.mock("../infra/tmp-omniagent-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOmniAgentTmpDir.mockReturnValue("/tmp/omniagent");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOmniAgentTmpDir.mockReturnValue("/tmp/omniagent-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "omniagent-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOmniAgentTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "omniagent-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "omniagent-preferred", "downloads")}${path.sep}`),
    );
  });
