...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".omniagent-profile-decorated");
}
 ...
 */
export function decorateOmniAgentProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OMNIAGENT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OMNIAGENT_BROWSER_COLOR).toUpperCase();
