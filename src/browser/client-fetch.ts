...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-omniagent-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-omniagent-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-omniagent-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the OmniAgent gateway (OmniAgent.app menubar, or \`${formatCliCommand("omniagent gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the OmniAgent browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the OmniAgent browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
