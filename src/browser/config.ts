import type { BrowserConfig, BrowserProfileConfig, OmniAgentConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OMNIAGENT_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "omniagent" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OMNIAGENT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "omniagent" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OMNIAGENT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OMNIAGENT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OmniAgent browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for omniagent/omniagent CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OmniAgentConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OMNIAGENT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "omniagent";
