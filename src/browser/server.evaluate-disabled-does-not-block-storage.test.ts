  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          omniagent: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OMNIAGENT_GATEWAY_PORT;
    process.env.OMNIAGENT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OMNIAGENT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OMNIAGENT_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.OMNIAGENT_GATEWAY_PASSWORD = prevGatewayPassword;
    }
