...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.OMNIAGENT_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/omniagent" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "omniagent",
        profiles: {
          omniagent: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOmniAgentChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.OMNIAGENT_GATEWAY_PORT;
    process.env.OMNIAGENT_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.OMNIAGENT_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.OMNIAGENT_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayToken === undefined) {
      ...
      process.env.OMNIAGENT_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.OMNIAGENT_GATEWAY_PASSWORD;
    } else {
      process.env.OMNIAGENT_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
