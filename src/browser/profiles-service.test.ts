...
vi.mock("./chrome.js", () => ({
  resolveOmniAgentUserDataDir: vi.fn(() => "/tmp/omniagent-test/omniagent/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOmniAgentUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "omniagent-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveOmniAgentUserDataDir).mockReturnValue(userDataDir);
