...
const RELAY_TOKEN_CONTEXT = "omniagent-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const OMNIAGENT_RELAY_BROWSER = "OmniAgent/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.OMNIAGENT_GATEWAY_TOKEN?.trim() || process.env.OPENCLAW_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or OMNIAGENT_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedOmniAgentRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === OMNIAGENT_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
