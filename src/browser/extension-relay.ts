...
const RELAY_AUTH_HEADER = "x-omniagent-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/OmniAgent-Extension-Relay",
          revision: "0",
          userAgent: "OmniAgent-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "OmniAgent/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedOmniAgentRelay({
