...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "omniagent_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "omniagent_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(actions.has("omniagent_cmdarg")).toBe(true);
    expect(options.has("omniagent_cmdarg")).toBe(true);
  });
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("omniagent_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("omniagent_cmdarg_ext:");
