...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OmniAgentPluginDefinition;
  register?: OmniAgentPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OmniAgentPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OmniAgentPluginDefinition;
...
export function loadOmniAgentPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOmniAgentPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "omniagent/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "omniagent/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OmniAgentPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as OmniAgentPluginModule;
    } catch (err) {
