...
import { loadOmniAgentPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `omniagent-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.OMNIAGENT_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "omniagent.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.OMNIAGENT_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadOmniAgentPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.OMNIAGENT_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OMNIAGENT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOmniAgentPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOmniAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOmniAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.OMNIAGENT_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ OMNIAGENT_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
