...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `omniagent-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "omniagent.plugin.json"), JSON.stringify(manifest), "utf-8");
}
