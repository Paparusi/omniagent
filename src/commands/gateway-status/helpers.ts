...
import type { OmniAgentConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: OmniAgentConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.OMNIAGENT_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.OMNIAGENT_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: OmniAgentConfig) {
