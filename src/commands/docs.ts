...
const SEARCH_TOOL = "https://docs.omniagent.ai/mcp.SearchOmniAgent";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.omniagent.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('omniagent docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.omniagent.ai/");
      runtime.log(`Search: ${formatCliCommand('omniagent docs "your query"')}`);
    }
    return;
  }
