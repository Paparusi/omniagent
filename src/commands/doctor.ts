...
function resolveMode(cfg: OmniAgentConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OmniAgent doctor");

  const root = await resolveOmniAgentPackageRoot({
  ...
  let cfg: OmniAgentConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("omniagent configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("omniagent config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("omniagent setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("omniagent doctor --fix")}" to apply changes.`);
  }
