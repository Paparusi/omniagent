...
export function requireOmniAgentAgentDir(): string {
  const agentDir = process.env.OMNIAGENT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OMNIAGENT_AGENT_DIR not set");
  }
  return agentDir;
}
