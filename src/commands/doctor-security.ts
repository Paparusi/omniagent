...
import type { OmniAgentConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { resolveDmAllowState } from "../security/dm-policy-shared.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: OmniAgentConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("omniagent security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("omniagent configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("omniagent config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("omniagent doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("omniagent config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('omniagent config set session.dmScope "per-channel-peer"') +
