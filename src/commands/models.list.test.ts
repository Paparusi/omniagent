...
const ensureOmniAgentModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/omniagent-agent/auth.json" });
const resolveOmniAgentAgentDir = vi.fn().mockReturnValue("/tmp/omniagent-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/omniagent-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureOmniAgentModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/omniagent-agent");
  });
