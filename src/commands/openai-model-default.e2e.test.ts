...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OmniAgentConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OmniAgentConfig) => { changed: boolean; next: OmniAgentConfig };
  defaultModel: string;
  overrideConfig: OmniAgentConfig;
  alreadyDefaultConfig: OmniAgentConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OmniAgentConfig) => config,
      applyDefaultConfig: (config: OmniAgentConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OmniAgentConfig = { agents: { defaults: {} } };
