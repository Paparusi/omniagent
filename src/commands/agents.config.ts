...
function resolveAgentName(cfg: OmniAgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: OmniAgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OmniAgentConfig): AgentSummary[] {
