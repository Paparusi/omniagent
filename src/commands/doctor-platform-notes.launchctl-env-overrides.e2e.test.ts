...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OMNIAGENT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OMNIAGENT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OMNIAGENT_GATEWAY_TOKEN");
    expect(message).not.toContain("OMNIAGENT_GATEWAY_PASSWORD");
  });
