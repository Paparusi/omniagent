...
export async function loadValidConfigOrThrow(): Promise<OmniAgentConfig> {
...
export async function updateConfig(
  mutator: (cfg: OmniAgentConfig) => OmniAgentConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OmniAgentConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OmniAgentConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("omniagent agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
