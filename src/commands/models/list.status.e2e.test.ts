...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOmniAgentAgentDir: vi.fn().mockReturnValue("/tmp/omniagent-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/omniagent-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/omniagent-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOmniAgentAgentDir: mocks.resolveOmniAgentAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/omniagent-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOmniAgentAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/omniagent-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/omniagent-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/omniagent-agent-custom");
