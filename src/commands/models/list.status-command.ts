...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOmniAgentAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("omniagent models auth setup-token")}\` or \`${formatCliCommand("omniagent configure")}\`.`
          : `Run \`${formatCliCommand("omniagent configure")}\` or set an API key env var.`;
