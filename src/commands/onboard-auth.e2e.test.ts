...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OMNIAGENT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("omniagent-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-oauth-sync-"));
    ...
    process.env.OMNIAGENT_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-oauth-nosync-"));
    process.env.OMNIAGENT_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OMNIAGENT_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-oauth-external-"));
    process.env.OMNIAGENT_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OMNIAGENT_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OMNIAGENT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("omniagent-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
