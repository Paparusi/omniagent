...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OmniAgent status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("omniagent security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("omniagent security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.omniagent.ai/faq");
  runtime.log("Troubleshooting: https://docs.omniagent.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("omniagent status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("omniagent logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("omniagent status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("omniagent gateway probe")}`);
  }
}
