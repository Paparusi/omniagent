...
export const resolveOmniAgentPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOmniAgentPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/omniagent.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveOmniAgentPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OMNIAGENT_STATE_DIR;
  originalUpdateInProgress = process.env.OMNIAGENT_UPDATE_IN_PROGRESS;
  process.env.OMNIAGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniagent-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OMNIAGENT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OMNIAGENT_UPDATE_IN_PROGRESS;
  } else {
    process.env.OMNIAGENT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
