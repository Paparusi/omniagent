...
beforeAll(() => {
  envSnapshot = captureEnv(["OMNIAGENT_PROFILE"]);
  process.env.OMNIAGENT_PROFILE = "isolated";
});
...
vi.mock("../infra/omniagent-root.js", () => ({
  resolveOmniAgentPackageRoot: vi.fn().mockResolvedValue("/tmp/omniagent"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/omniagent/pnpm-lock.yaml",
      markerPath: "/tmp/omniagent/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("OmniAgent status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("omniagent status --all") ||
          l.includes("omniagent --profile isolated status --all") ||
          l.includes("omniagent status --all") ||
          l.includes("omniagent --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OMNIAGENT_GATEWAY_TOKEN = "abcd1234";
    try {
