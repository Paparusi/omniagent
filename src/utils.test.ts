...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("omniagent-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("omniagent-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("omniagent-lid-a-", (first) => {
      withTempDirSync("omniagent-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.omniagent when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "omniagent-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OMNIAGENT_HOME over HOME", () => {
    vi.stubEnv("OMNIAGENT_HOME", "/srv/omniagent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/omniagent-home"));
...
describe("shortenHomePath", () => {
  it("uses $OMNIAGENT_HOME prefix when OMNIAGENT_HOME is set", () => {
    vi.stubEnv("OMNIAGENT_HOME", "/srv/omniagent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/omniagent-home")}/.omniagent/omniagent.json`)).toBe(
      "$OMNIAGENT_HOME/.omniagent/omniagent.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OMNIAGENT_HOME replacement when OMNIAGENT_HOME is set", () => {
    vi.stubEnv("OMNIAGENT_HOME", "/srv/omniagent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/omniagent-home")}/.omniagent/omniagent.json`),
    ).toBe("config: $OMNIAGENT_HOME/.omniagent/omniagent.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/omniagent")).toBe(path.resolve(os.homedir(), "omniagent"));
  });
  ...
  it("prefers OMNIAGENT_HOME for tilde expansion", () => {
    vi.stubEnv("OMNIAGENT_HOME", "/srv/omniagent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/omniagent")).toBe(path.resolve("/srv/omniagent-home", "omniagent"));
