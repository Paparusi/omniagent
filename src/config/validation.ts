...
import type { OmniAgentConfig, ConfigValidationIssue } from "./types.js";
import { OmniAgentSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: OmniAgentConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OmniAgentConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OmniAgentSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OmniAgentConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OmniAgentConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OmniAgentConfig,
  };
}
