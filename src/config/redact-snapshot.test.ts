...
import type { ConfigFileSnapshot } from "./types.omniagent.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.omniagent/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/omniagent/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/omniagent/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/omniagent/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/omniagent/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OmniAgentSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OmniAgentConfig";
    const hints = mapSensitivePaths(OmniAgentSchema, "", {});
