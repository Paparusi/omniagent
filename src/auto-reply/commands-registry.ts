...
export function isCommandEnabled(cfg: OmniAgentConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OmniAgentConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OmniAgentConfig),
...
export function getCommandDetection(_cfg?: OmniAgentConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OmniAgentConfig) {
