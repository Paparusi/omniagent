...
function createReplyConfig(home: string, streamMode?: "block"): OmniAgentConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("omniagent-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("OMNIAGENT_TEST_FAST", "1");
