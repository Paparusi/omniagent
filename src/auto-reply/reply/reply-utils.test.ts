...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bomniagent\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: {
          identityName: "OmniAgent",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[OmniAgent] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
