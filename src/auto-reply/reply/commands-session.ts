...
import { scheduleGatewaySigusr1Restart, triggerOmniAgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting OmniAgent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOmniAgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting OmniAgent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
