...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: OmniAgentConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: OmniAgentConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.OMNIAGENT_TEST_FAST === "1") {
