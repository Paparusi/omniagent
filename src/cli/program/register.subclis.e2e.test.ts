...
describe("registerSubCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.OMNIAGENT_DISABLE_LAZY_SUBCOMMANDS;
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "omniagent", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "omniagent"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "omniagent", "nodes", "list"], "omniagent");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "omniagent", "acp", "--help"], "omniagent");
    ...
    await program.parseAsync(["node", "omniagent", "acp"], { from: "user" });
    expect(registerAcpCli).toHaveBeenCalledTimes(1);
    expect(acpAction).toHaveBeenCalledTimes(1);
  });
});
