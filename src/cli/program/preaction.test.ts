...
beforeEach(() => {
  ...
  originalHideBanner = process.env.OMNIAGENT_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.OMNIAGENT_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.OMNIAGENT_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("omniagent");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "omniagent", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("omniagent-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "omniagent", "message", "send"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "omniagent", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "omniagent", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "omniagent", "--version"],
    });
  ...
  it("hides banner when OMNIAGENT_HIDE_BANNER is truthy", async () => {
    process.env.OMNIAGENT_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "omniagent", "status"],
    });
