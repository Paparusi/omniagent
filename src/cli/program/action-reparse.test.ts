...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "omniagent", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "omniagent", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "omniagent", "status"]);
  });
