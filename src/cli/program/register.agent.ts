...
${formatHelpExamples([
  ['omniagent agent --to +15555550123 --message "status update"', "Start a new session."],
  ['omniagent agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'omniagent agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'omniagent agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['omniagent agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'omniagent agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.omniagent.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.omniagent.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['omniagent agents set-identity --agent main --name "OmniAgent" --emoji "🦞"', "Set name + emoji."],
  ["omniagent agents set-identity --agent main --avatar avatars/omniagent.png", "Set avatar path."],
  [
    "omniagent agents set-identity --workspace ~/.omniagent/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "omniagent agents set-identity --identity-file ~/.omniagent/workspace/IDENTITY.md --agent main",
