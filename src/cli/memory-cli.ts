    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["omniagent memory status", "Show index and provider status."],
          ["omniagent memory index --force", "Force a full reindex."],
          ['omniagent memory search --query "deployment notes"', "Search indexed memory entries."],
          ["omniagent memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.omniagent.ai/cli/memory")}\n`,
    );
