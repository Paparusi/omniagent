...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniagent-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniagent-bundled-skills-test-"));
    process.env.OMNIAGENT_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
