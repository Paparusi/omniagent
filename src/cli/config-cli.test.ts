...
import type { ConfigFileSnapshot, OmniAgentConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `omniagent config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: OmniAgentConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OmniAgentConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: OmniAgentConfig, config: OmniAgentConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: OmniAgentConfig = {
        agents: {
      ...
      const runtimeMerged: OmniAgentConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: OmniAgentConfig = { gateway: { port: 18789 } };
