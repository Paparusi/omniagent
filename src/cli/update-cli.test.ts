...
import type { OmniAgentConfig, ConfigFileSnapshot } from "../config/types.omniagent.js";
...
vi.mock("../infra/omniagent-root.js", () => ({
  resolveOmniAgentPackageRoot: vi.fn(),
}));
...
const { resolveOmniAgentPackageRoot } = await import("../infra/omniagent-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/omniagent-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OmniAgentConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/omniagent-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOmniAgentPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("omniagent-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOmniAgentPackageRoot).mockReset();
    ...
    vi.mocked(resolveOmniAgentPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("omniagent");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/omniagent-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "omniagent-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OmniAgent update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OmniAgentConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOmniAgentPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("omniagent-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OMNIAGENT_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        ...
        expect(process.env.OMNIAGENT_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("omniagent-update-wizard");
    await withEnvAsync({ OMNIAGENT_GIT_DIR: tempDir }, async () => {
      setTty(true);
