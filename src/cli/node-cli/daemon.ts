...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("omniagent node install"), formatCliCommand("omniagent node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("omniagent node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OMNIAGENT_LOG_PREFIX: baseEnv.OMNIAGENT_LOG_PREFIX ?? "node",
