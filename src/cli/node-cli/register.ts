    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "omniagent node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["omniagent node status", "Check node host service status."],
          ["omniagent node install", "Install the node host service."],
          ["omniagent node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.omniagent.ai/cli/node")}\n`,
    );
