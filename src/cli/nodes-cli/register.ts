    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["omniagent nodes status", "List known nodes with live status."],
          ["omniagent nodes pairing pending", "Show pending node pairing requests."],
          ['omniagent nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["omniagent nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.omniagent.ai/cli/nodes")}\n`,
    );
