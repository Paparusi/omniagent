...
function appendOmniHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("omniagent skills list")}\` to see all skills.`
      : "No skills found.";
    return appendOmniHubHint(message, opts.json);
  }
  ...
  return appendOmniHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendOmniHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("omniagent skills list")}\` to see available skills.`,
      opts.json,
    );
  }
