...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio omniagent.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.omniagent.internal",
        "  ws: ws://studio.omniagent.internal:18789",
      ],
    },
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ OMNIAGENT_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
