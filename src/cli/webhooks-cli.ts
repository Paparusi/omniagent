...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.omniagent.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OmniAgent hooks")
    ...
    .option("--hook-url <url>", "OmniAgent hook URL")
    .option("--hook-token <token>", "OmniAgent hook token")
