...
import { resolveOmniAgentPackageRoot } from "../../infra/omniagent-root.js";
...
const OMNIAGENT_REPO_URL = "https://github.com/Paparusi/omniagent.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "omniagent";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("omniagent@")) {
    return trimmed.slice("omniagent@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.OMNIAGENT_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOmniAgentPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OMNIAGENT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OMNIAGENT_GIT_DIR points at a non-git directory: ${params.dir}. Set OMNIAGENT_GIT_DIR to an empty folder or an omniagent checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OMNIAGENT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "omniagent.mjs");
