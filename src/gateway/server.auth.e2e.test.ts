...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.OMNIAGENT_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.OMNIAGENT_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.OMNIAGENT_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.OMNIAGENT_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers service version fallback in hello-ok payload", async () => {
      await withRuntimeVersionEnv(
        {
          OMNIAGENT_VERSION: " ",
          OMNIAGENT_SERVICE_VERSION: "2.4.6-service",
    ...
    test("connect (req) handshake prefers OMNIAGENT_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
          OMNIAGENT_VERSION: "9.9.9-cli",
          OMNIAGENT_SERVICE_VERSION: "2.4.6-service",
          npm_package_version: "1.0.0-package",
        },
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `omniagent-test-device-${randomUUID()}.json`),
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.OMNIAGENT_GATEWAY_TOKEN;
      process.env.OMNIAGENT_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "omniagent-device-scope-"));
  ...
  test("rejects scope escalation from legacy paired metadata", async () => {
    ...
    let ws2: WebSocket | undefined;
    try {
      const identityDir = await mkdtemp(join(tmpdir(), "omniagent-device-legacy-"));
      const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
      const client = {
        id: GATEWAY_CLIENT_NAMES.TEST,
        version: "1.0.0",
