...
import { createOmniAgentTools } from "../agents/omniagent-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-omniagent-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-omniagent-account-id")?.trim() || undefined;
  ...
  const allTools = createOmniAgentTools({
