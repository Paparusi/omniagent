...
type GetReplyFromConfigFn = (
  ...
  configOverride?: OmniAgentConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `omniagent-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OMNIAGENT_CONFIG_PATH = path.join(root, "omniagent.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "omniagent.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "omniagent-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadOmniAgentPlugins: () => pluginRegistryState.registry,
  };
});
