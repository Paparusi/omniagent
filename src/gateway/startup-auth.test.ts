...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: OmniAgentConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: OmniAgentConfig) {
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          OMNIAGENT_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
