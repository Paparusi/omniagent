    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "omniagent", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-omniagent-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-omniagent-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "omniagent", messages: [{ role: "user", content: "hi" }] },
          {
            "x-omniagent-agent-id": "beta",
            "x-omniagent-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
