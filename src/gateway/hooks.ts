...
export function resolveHooksConfig(cfg: OmniAgentConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OmniAgentConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-omniagent-token"] === "string"
      ? req.headers["x-omniagent-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
