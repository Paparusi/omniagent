...
describe("gateway server models + voicewake", () => {
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("omniagent-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["omniagent", "claude", "computer"]);
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".omniagent", "settings", "voicewake.json"), "utf8"),
  ...
  test("models.list filters to allowlisted configured models by default", async () => {
    const configPath = process.env.OMNIAGENT_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing OMNIAGENT_CONFIG_PATH");
    }
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    await withEnvAsync({ OMNIAGENT_GATEWAY_TOKEN: "secret" }, async () => {
      testTailnetIPv4.value = "100.64.0.1";
      testState.gatewayBind = "lan";
      const canvasPort = await getFreePort();
      testState.canvasHostPort = canvasPort;
      await withEnvAsync({ OMNIAGENT_CANVAS_HOST_PORT: String(canvasPort) }, async () => {
