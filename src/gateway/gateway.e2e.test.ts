...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "omniagent");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.omniagent-tool-probe.${nonceA}.txt`);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-wizard-home-"));
      process.env.HOME = tempHome;
