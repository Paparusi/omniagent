...
beforeAll(async () => {
  previousToken = process.env.OMNIAGENT_GATEWAY_TOKEN;
  process.env.OMNIAGENT_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.OMNIAGENT_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "omniagent-ios", platform: "ios" },
  { clientId: "omniagent-android", platform: "android" },
