...
export function buildGatewayConnectionDetails(
  options: { config?: OmniAgentConfig; url?: string; configPath?: string } = {},
...
function resolveGatewayCredentials(context: ResolvedGatewayCallContext): {
  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.OMNIAGENT_GATEWAY_TOKEN) ||
  ...
  const password =
    ...
    (!context.urlOverride
      ? trimToUndefined(process.env.OMNIAGENT_GATEWAY_PASSWORD) ||
