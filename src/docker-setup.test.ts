...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "omniagent-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OMNIAGENT_GATEWAY_TOKEN: "test-token",
    OMNIAGENT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OMNIAGENT_WORKSPACE_DIR: join(sandbox.rootDir, "omniagent"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        OMNIAGENT_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        OMNIAGENT_EXTRA_MOUNTS: undefined,
        OMNIAGENT_HOME_VOLUME: "omniagent-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OMNIAGENT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OMNIAGENT_EXTRA_MOUNTS=");
    expect(envFile).toContain("OMNIAGENT_HOME_VOLUME=omniagent-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("omniagent-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("omniagent-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OMNIAGENT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });

  it("rejects injected multiline OMNIAGENT_EXTRA_MOUNTS values", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        OMNIAGENT_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
    ...
    expect(result.stderr).toContain("OMNIAGENT_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid OMNIAGENT_EXTRA_MOUNTS mount format", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        OMNIAGENT_EXTRA_MOUNTS: "bad mount spec",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
  ...
  it("rejects invalid OMNIAGENT_HOME_VOLUME names", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        OMNIAGENT_HOME_VOLUME: "bad name",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OMNIAGENT_HOME_VOLUME must match");
  });
