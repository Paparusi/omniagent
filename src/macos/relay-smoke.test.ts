...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { OMNIAGENT_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { OMNIAGENT_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports OMNIAGENT_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { OMNIAGENT_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { OMNIAGENT_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { OMNIAGENT_SMOKE: "nope" })).toBe("qr");
  });
});
