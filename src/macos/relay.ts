...
async function main() {
  ...
  const { ensureOmniAgentCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[omniagent] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
