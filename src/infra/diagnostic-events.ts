...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __omniagentDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__omniagentDiagnosticEventsState) {
    globalStore.__omniagentDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__omniagentDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OmniAgentConfig): boolean {
