...
function candidateBinDirs(opts: EnsureOmniAgentPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `omniagent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "omniagent");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OMNIAGENT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "omniagent"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `omniagent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOmniAgentCliOnPath(opts: EnsureOmniAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OMNIAGENT_PATH_BOOTSTRAPPED)) {
    return;
  }
