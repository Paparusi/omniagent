...
import { resolveOmniAgentPackageRoot, resolveOmniAgentPackageRootSync } from "./omniagent-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOmniAgentPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "omniagent" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOmniAgentPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // omniagent.mjs or dist/<bundle>.js
