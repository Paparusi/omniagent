...
vi.mock("./install-source-utils.js", async (importOriginal) => {
  ...
  return {
    ...
    withTempDir: vi.fn(async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
      return await fn("/tmp/omniagent-npm-pack-install-test");
    }),
...
describe("installFromNpmSpecArchive", () => {
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "omniagent-test-",
      spec: "@omniagent/test@1.0.0",
      timeoutMs: 1000,
      installFromArchive,
    });
    ...
    expect(withTempDir).toHaveBeenCalledWith("omniagent-test-", expect.any(Function));
  });

  it("returns resolution metadata and installer result on success", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/omniagent-test.tgz",
      metadata: {
        name: "@omniagent/test",
        version: "1.0.0",
        resolvedSpec: "@omniagent/test@1.0.0",
    ...
    expect(result.npmResolution.resolvedSpec).toBe("@omniagent/test@1.0.0");
    expect(result.npmResolution.resolvedAt).toBeTruthy();
    expect(installFromArchive).toHaveBeenCalledWith({ archivePath: "/tmp/omniagent-test.tgz" });
  });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @omniagent/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and proceeds on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @omniagent/test@1.0.0: expected sha512-old, got sha512-new",
    );
  });
  ...
  it("returns installer failures to callers for domain-specific handling", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/omniagent-test.tgz",
      metadata: { resolvedSpec: "@omniagent/test@1.0.0", integrity: "sha512-same" },
    });
