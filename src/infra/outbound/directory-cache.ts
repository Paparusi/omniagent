...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OmniAgentConfig | null = null;
  ...
  get(key: string, cfg: OmniAgentConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OmniAgentConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: OmniAgentConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: OmniAgentConfig): void {
