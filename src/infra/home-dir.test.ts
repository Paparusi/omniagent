...
describe("resolveEffectiveHomeDir", () => {
  it("prefers OMNIAGENT_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/omniagent-home"),
    );
  });
  ...
  it("expands OMNIAGENT_HOME when set to ~", () => {
    const env = {
      OMNIAGENT_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for OMNIAGENT_HOME", () => {
    const result = resolveRequiredHomeDir(
      { OMNIAGENT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when OMNIAGENT_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ OMNIAGENT_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { OMNIAGENT_HOME: "/srv/omniagent-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/omniagent-home")}/x`);
  });
