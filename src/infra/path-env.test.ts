...
let ensureOmniAgentCliOnPath: typeof import("./path-env.js").ensureOmniAgentCliOnPath;

describe("ensureOmniAgentCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OMNIAGENT_PATH_BOOTSTRAPPED",
    "OMNIAGENT_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOmniAgentCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling omniagent exists", () => {
    const tmp = abs("/tmp/omniagent-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "omniagent");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OMNIAGENT_PATH_BOOTSTRAPPED;

    ensureOmniAgentCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.OMNIAGENT_PATH_BOOTSTRAPPED = "1";
    ensureOmniAgentCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/omniagent-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "omniagent");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/omniagent-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "omniagent");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "omniagent");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/omniagent-path/case-linuxbrew");
