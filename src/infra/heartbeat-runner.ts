...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OmniAgentConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OmniAgentConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OmniAgentConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OmniAgentConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OmniAgentConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OmniAgentConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OmniAgentConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
