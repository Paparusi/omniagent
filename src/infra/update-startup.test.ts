...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OMNIAGENT_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OMNIAGENT_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOmniAgentPackageRoot } = await import("./omniagent-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveOmniAgentPackageRoot).mockResolvedValue("/opt/omniagent");
