...
export function triggerOmniAgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OMNIAGENT_SYSTEMD_UNIT,
        process.env.OMNIAGENT_PROFILE,
      );
  ...
  const label =
    process.env.OMNIAGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OMNIAGENT_PROFILE);
