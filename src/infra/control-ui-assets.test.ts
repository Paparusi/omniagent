...
vi.mock("./omniagent-root.js", () => ({
  resolveOmniAgentPackageRoot: vi.fn(async () => null),
  resolveOmniAgentPackageRootSync: vi.fn(() => null),
}));
...
let omniagentRoot: typeof import("./omniagent-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    omniagentRoot = await import("./omniagent-root.js");
  });
  ...
  it("uses resolveOmniAgentPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/omniagent");
    (
      omniagentRoot.resolveOmniAgentPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/omniagent"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "omniagent" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "omniagent.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-omniagent");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/omniagent-bundle");
    (
      omniagentRoot.resolveOmniAgentPackageRootSync as unknown as ReturnType<typeof vi.fn>
