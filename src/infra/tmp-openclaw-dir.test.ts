...
import { POSIX_OMNIAGENT_TMP_DIR, resolvePreferredOmniAgentTmpDir } from "./tmp-omniagent-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOmniAgentTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `omniagent-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOmniAgentTmpDir({
    accessSync,
...
describe("resolvePreferredOmniAgentTmpDir", () => {
  it("prefers /tmp/omniagent when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OMNIAGENT_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/omniagent when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OMNIAGENT_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/omniagent when /tmp/omniagent is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/omniagent when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/omniagent is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/omniagent is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/omniagent is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
