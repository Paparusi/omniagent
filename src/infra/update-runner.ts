...
const DEFAULT_PACKAGE_NAME = "omniagent";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "omniagent.mjs");
    ...
    const doctorStep = await runStep(
      step("omniagent doctor", doctorArgv, gitRoot, { OMNIAGENT_UPDATE_IN_PROGRESS: "1" }),
    );
