...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await makeTempDir("omniagent-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await makeTempDir("omniagent-fs-safe-root-");
    const outside = await makeTempDir("omniagent-fs-safe-outside-");
