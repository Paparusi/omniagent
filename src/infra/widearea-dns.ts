...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OMNIAGENT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*omniagent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "omniagent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "omniagent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "omniagent-gw");
  ...
  records.push(`_omniagent-gw._tcp IN PTR ${instanceLabel}._omniagent-gw._tcp`);
  records.push(`${instanceLabel}._omniagent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._omniagent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; omniagent-content-hash: ${contentHash}\n${contentBody}`;
}
