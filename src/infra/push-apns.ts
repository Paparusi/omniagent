...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OMNIAGENT_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OMNIAGENT_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OMNIAGENT_APNS_TEAM_ID and OMNIAGENT_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OMNIAGENT_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OMNIAGENT_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OMNIAGENT_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OMNIAGENT_APNS_PRIVATE_KEY_P8 or OMNIAGENT_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OMNIAGENT_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOmniAgentPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    omniagent: createOmniAgentPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
