...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OMNIAGENT_GATEWAY_PORT).toBe("18789");
    expect(env.OMNIAGENT_GATEWAY_TOKEN).toBe("secret");
    expect(env.OMNIAGENT_SERVICE_MARKER).toBe("omniagent");
    expect(env.OMNIAGENT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OMNIAGENT_SERVICE_VERSION).toBe("string");
    expect(env.OMNIAGENT_SYSTEMD_UNIT).toBe("omniagent-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OMNIAGENT_LAUNCHD_LABEL).toBe("ai.omniagent.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OMNIAGENT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OMNIAGENT_SYSTEMD_UNIT).toBe("omniagent-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OMNIAGENT_LAUNCHD_LABEL).toBe("ai.omniagent.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".omniagent"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OMNIAGENT_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".omniagent-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OMNIAGENT_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".omniagent"));
  });

  it("uses OMNIAGENT_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OMNIAGENT_STATE_DIR: "/var/lib/omniagent" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/omniagent"));
  });

  it("expands ~ in OMNIAGENT_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OMNIAGENT_STATE_DIR: "~/omniagent-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/omniagent-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OMNIAGENT_STATE_DIR: "C:\\State\\omniagent" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\omniagent");
  });
});
