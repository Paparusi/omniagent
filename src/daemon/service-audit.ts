...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.OMNIAGENT_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service OMNIAGENT_GATEWAY_TOKEN does not match gateway.auth.token in omniagent.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `omniagent gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
