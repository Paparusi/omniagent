...
describe("canvas host state dir defaults", () => {
  it("uses OMNIAGENT_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("omniagent-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OmniAgent Canvas");
      } finally {
        await handler.close();
      }
    });
