...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.omniagentCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.omniagentCanvasA2UIAction &&
        typeof window.omniagentCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.omniagentSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("omniagent:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OmniAgent node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.omniagentSendUserAction === "function"
        ? window.omniagentSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OMNIAGENT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OMNIAGENT_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OmniAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
