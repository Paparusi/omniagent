...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OMNIAGENT_HOME for tilde expansion", () => {
    vi.stubEnv("OMNIAGENT_HOME", "/srv/omniagent-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/omniagent-home", "cron", "jobs.json"));
  });
});
