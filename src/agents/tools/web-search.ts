...
type WebSearchConfig = NonNullable<OmniAgentConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OmniAgentConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("omniagent configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.omniagent.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://omniagent.ai",
      "X-Title": "OmniAgent Web Search",
    },
