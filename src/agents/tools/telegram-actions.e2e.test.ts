...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OmniAgentConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OmniAgentConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OmniAgentConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OmniAgentConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: OmniAgentConfig, accountId = "media") {
    await handleTelegramAction(
