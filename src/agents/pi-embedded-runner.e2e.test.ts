...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies OmniAgentConfig;

const ensureModels = (cfg: OmniAgentConfig) => ensureOmniAgentModelsJson(cfg, agentDir) as unknown;
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("persists prompt transport errors as transcript entries", async () => {
    ...
    const promptErrorEntry = entries.find(
      (entry) => entry.type === "custom" && entry.customType === "omniagent:prompt-error",
