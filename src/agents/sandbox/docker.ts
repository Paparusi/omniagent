...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "omniagent.sandbox=1");
  args.push("--label", `omniagent.sessionKey=${params.scopeKey}`);
  args.push("--label", `omniagent.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `omniagent.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "omniagent.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`omniagent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`omniagent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("omniagent sandbox recreate --all");
}
