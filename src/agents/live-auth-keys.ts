...
const GOOGLE_LIVE_SINGLE_KEY = "OMNIAGENT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OMNIAGENT_LIVE_ANTHROPIC_KEY",
    listVar: "OMNIAGENT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OMNIAGENT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OMNIAGENT_LIVE_${base}_KEY`;
