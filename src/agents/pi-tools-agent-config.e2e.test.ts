...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniagent-pi-tools-"));
...
*** End Patch`;
    ...
    const cfg: OmniAgentConfig = {
      ...
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/omniagent-restricted",
            tools: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createOmniAgentCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createOmniAgentCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createOmniAgentCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createOmniAgentCodingTools({
      config: cfg,
    ...
    const bobTools = createOmniAgentCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createOmniAgentCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOmniAgentCodingTools({
      config: cfg,
