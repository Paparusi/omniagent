...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureOmniAgentModelsJson(validated.config);

      const modelPath = path.join(resolveOmniAgentAgentDir(), "models.json");
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureOmniAgentModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
