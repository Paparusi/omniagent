...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OMNIAGENT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OMNIAGENT_HOME: "/srv/omniagent-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/omniagent-home"), ".omniagent", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".omniagent", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("omniagent-workspace-");
