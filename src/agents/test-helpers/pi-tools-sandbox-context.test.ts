...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("omniagent-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("omniagent-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("omniagent-sbx-");
