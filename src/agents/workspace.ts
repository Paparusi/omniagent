...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OMNIAGENT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".omniagent", `workspace-${profile}`);
  }
  return path.join(home, ".omniagent", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".omniagent";
