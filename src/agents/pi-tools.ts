...
function resolveExecConfig(params: { cfg?: OmniAgentConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OmniAgentConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOmniAgentCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOmniAgentReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
