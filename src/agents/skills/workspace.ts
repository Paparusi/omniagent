...
function resolveSkillsLimits(config?: OmniAgentConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "omniagent-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "omniagent-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveOmniAgentMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OmniAgentConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`omniagent skills check\` to audit.`
    : "";
