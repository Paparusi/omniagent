...
const defaultTools = createOmniAgentCodingTools({ senderIsOwner: true });
...
describe("createOmniAgentCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createOmniAgentCodingTools({
      config,
    ...
    const anthropicTools = createOmniAgentCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createOmniAgentCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createOmniAgentCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
