...
function listAgents(cfg: OmniAgentConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OmniAgentConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OmniAgentConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OmniAgentConfig, EvaluatedBindingsCache>();
