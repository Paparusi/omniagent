...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".omniagent");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OMNIAGENT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OMNIAGENT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OMNIAGENT_STATE_DIR.
