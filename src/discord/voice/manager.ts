...
function resolveVoiceTtsConfig(params: { cfg: OmniAgentConfig; override?: TtsConfig }): {
  cfg: OmniAgentConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOmniAgentTmpDir(), "discord-voice-"));
