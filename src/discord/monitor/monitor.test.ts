...
describe("agent components", () => {
  const createCfg = (): OmniAgentConfig => ({}) as OmniAgentConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OmniAgentConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/omniagent-sessions-test.json");
  });
