...
const resolveStorePath = vi.fn(() => "/tmp/omniagent-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/omniagent-discord-process-test-sessions.json");
  ...
  it("streams block previews using draft chunking", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/omniagent-discord-process-test-sessions.json" },
        channels: {
          discord: {
