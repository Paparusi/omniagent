...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ OMNIAGENT_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.OMNIAGENT_BASE_ENV ?? "") + "|" + (process.env.OMNIAGENT_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { OMNIAGENT_TEST_ENV: "ok" },
        },
      );
